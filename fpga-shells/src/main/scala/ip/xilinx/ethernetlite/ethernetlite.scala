...
class EthernetLite(implicit p:Parameters) extends LazyModule
{
  val device = new SimpleDevice("ethernet", Seq("xlnx,axi-ethernet-lite-1.00.a"))

  val slave = AXI4SlaveNode(Seq(AXI4SlavePortParameters(
    slaves = Seq(AXI4SlaveParameters(
      address       = List(AddressSet(0x60000000L, 0x1fff)),
